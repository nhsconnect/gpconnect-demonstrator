        ...
        if(nhsNumber == null || nhsNumber.isEmpty() || !NhsCodeValidator.nhsNumberValid(nhsNumber)){
            ...
            try{
                ...
                if(careProviderPractitionerList.size() > 0){
                    ...
                    // Try and get Practitioner details but if we fail to find any an exception will be thrown which we need to catch and
                    // convert to an operational outcome error in the bundle
                    try{
                        Entry practitionerEntry = new Entry();
                        Practitioner practitioner = practitionerResourceProvider.getPractitionerById(new IdDt(practitionerReferenceStr));
